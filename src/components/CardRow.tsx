  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden" style={{ height: 210 }}>
        ...
        {/* Duration */}
        <div className="absolute bottom-9 right-3 px-1.5 py-0.5 rounded text-white text-[10px]  font-medium"
      ...
      <div className="px-5 pt-4 pb-1">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[11px] font-semibold tracking-widest uppercase" style={{ color: '#aaa' }}>{video.channel}</span>
          <YTIcon />
        </div>
        <h2 className="font-bold text-[19px] leading-snug tracking-tight" style={{ color: '#0f0f0f' }}>
          {video.title}
        </h2>
        <p className="text-[13px] leading-relaxed mt-1.5" style={{ color: '#606060',
      ...
      <div className="px-5 mt-3">
        <div className="relative h-[3px] rounded-full cursor-pointer" style={{ background: '#e9e9e9' }} onClick={seek}>
          <div className="absolute left-0 top-0 h-full rounded-full" style={{ width: `${progress * 100}%`, background: '#FF0000', transition: 'width 0.5s linear' }} />
          <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full shadow-md" style={{ left: `calc(${progress * 100}% - 6px)`, background: '#FF0000' }} />
        </div>
      ...
      <div className="flex items-center justify-between px-5 pb-5 mt-2">
        ...
        <div className="flex gap-5">
          <div>
            <p className="text-[16px] font-bold" style={{ color: '#0f0f0f' }}>{Math.round(progress * 100)}%</p>
            <p className="text-[10px] mt-0.5" style={{ color: '#bbb' }}>Progress</p>
          </div>
          <div>
            <p className="text-[16px] font-bold" style={{ color: '#0f0f0f' }}>{formatTime(elapsed)}</p>
            <p className="text-[10px] mt-0.5" style={{ color: '#bbb' }}>Watched</p>
          </div>
        </div>

        {/* Pill controls */}
        <div className="flex items-center gap-0.5 rounded-full px-2 py-1.5" style={{ background: '#f2f2f2' }}>
          {/* Rewind 10s */}
